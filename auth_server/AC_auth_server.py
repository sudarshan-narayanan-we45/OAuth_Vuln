...
def signin():
  ...
  authorization_code = generate_authorization_code(client_id, redirect_url, username)
...
def exchange_for_token():
  ...
  data = verify_authorization_code(authorization_code, client_id, redirect_url)
  if data == "False":
    return json.dumps({
      "error": "access_denied"
    }), 400
